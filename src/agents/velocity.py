...
from ..core.prompts import get_velocity_analysis_agent_prompt
from ..tools.transactions import VelocityTransactions

# Load environment variables
load_dotenv()
OPENAI_API_KEY = os.getenv("OPENAI_API_KEY")

# Velocity Analysis Agent Class
class VelocityAnalysisAgent:
    def __init__(self, alert: Alert, transaction_data: pd.DataFrame):
        self.alert = alert
        self.config = self._get_custom_config()
        self.transaction_data = transaction_data
        self.tools = [VelocityTransactions(transaction_data)]
        self.agent = AssistantAgent(
            name="velocity_analysis_agent",
    ...
    def _get_system_message(self):
        return get_velocity_analysis_agent_prompt(self.config)
